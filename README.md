...
support batching prompts/tasks and processing them in a controlled, stateful way.

## What’s included

- WorkQueueService: A registry service that manages a FIFO queue and lifecycle for processing work items.
- chatCommands.queue: A chat command (/queue) to add, list, remove, start, step through, skip, run, and finish queued
  items.
...
WorkQueueService is a simple queue manager with optional capacity limits and minimal lifecycle. It stores arbitrary work
items (commonly { name, input, currentMessage }) and exposes queue operations.
...
The /queue command manages a queue of chat prompts (strings/messages) and coordinates execution in the chat UI. It
integrates with:
