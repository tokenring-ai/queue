...
export async function execute(remainder, registry) {
 ...
 switch (action) {
  case "add": {
   ...
   chatService.systemLine((`Added to queue. Queue length: ${workQueueService.size()}`));
   break;
  }
  case "remove": {
   ...
   if (isNaN(idx) || idx < 0 || idx >= workQueueService.size()) {
    chatService.errorLine(("Usage: /queue remove <index>  (index starts from 0)"));
    return;
   }
   const removed = workQueueService.splice(idx, 1);
   chatService.systemLine((`Removed \"${removed.name}\" from queue. Remaining: ${workQueueService.size()}`));
   break;
  }
  ...
  case "details": {
   ...
   if (isNaN(idx) || idx < 0 || idx >= workQueueService.size()) {
    chatService.errorLine(("Usage: /queue details <index>  (index starts from 0)"));
    return;
   }
  ...
  case "start": {
   ...
   if (workQueueService.started()) {
    chatService.systemLine(("Queue already started. Use /queue next to load the next item in the queue, or queue done to end the queue."));
    return;
   }

   workQueueService.setInitialMessage(chatMessageStorage.getCurrentMessage());
   workQueueService.start();


   await checkpoint.execute("create Start of queue operation", registry);
   chatService.systemLine(("Queue started, use /queue next to start working on the first item in the queue, or /queue done to end the queue."));
   break;
  }
  case "next":
  case "done": {
   if (!workQueueService.started()) {
    chatService.systemLine(("Queue not started. Use /queue start to start the queue."));
    return;
   }

   const currentItem = workQueueService.getCurrentItem();

   await checkpoint.execute(`create End of queue operation: ${currentItem.name}`, registry);

   if (action === "done" || workQueueService.isEmpty()) {
    chatMessageStorage.setCurrentMessage(workQueueService.getInitialMessage());
   ...
   chatService.systemLine((`Queue Item loaded: ${newItem.name} Use /queue run to run the queue item, and /queue next|skip|done to move on to the next item.`));
  ...
  case "skip": {
   ...
   const currentItem = workQueueService.getCurrentItem();
   if (!currentItem) {
    chatService.systemLine(("No queue item loaded. Use /queue next to load the next item in the queue, or queue done to end the queue."));
    return;
   }

   workQueueService.queue(currentItem);
   workQueueService.setCurrentItem(null);
   chatService.systemLine(("Queue item skipped. It has been added to the end of the queue in case you would like to run it later, and you can use /queue next to load the next item in the queue, or /queue done to end the queue."));
   break;
  }
  case "run": {
   if (!workQueueService.started()) {
   ...
   chatMessageStorage.setCurrentMessage(currentMessage ?? workQueueService.getInitialMessage());
   ...
   try {
    ...
    chatService.errorLine((`Error running queued prompt: ${error.message || error}`));
   }
   break;
  }
