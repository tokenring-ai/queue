...
describe('WorkQueueService', () => {
  ...
  it('should register and unregister commands during init and deinit', async () => {
    ...
    // Verify
    expect(mockServices.requireFirstServiceByType).toHaveBeenCalledWith(ChatCommandRegistry);
    expect(mockChatCommandService.registerCommand).toHaveBeenCalledWith('queue', expect.any(Object));
    
    // Execute deinit
    await workQueueService.deinit(mockServices);
    
    // Verify
    expect(mockChatCommandService.unregisterCommand).toHaveBeenCalledWith('queue');
  });
